...
{
    public struct VarUInt32 : ISerializable, IConvertible
    {
        private uint Value;
        private VarUInt32(uint value)
        {
            Value = value;
        }
        ...
        public async Task DeserializeAsync(SerialStream stream, CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                position += 7;
                if (position >= sizeof(uint) * 8) throw new ProtocolViolationException($"{nameof(VarUInt32)} size exceeded!");
            }
        }
        ...
        public static implicit operator uint(VarUInt32 data)
        {
            return data.Value;
        }
        public static implicit operator VarUInt32(uint data)
        {
            return new(data);
        }
    }
