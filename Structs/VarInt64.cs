...
{
    public struct VarInt64 : ISerializable, IConvertible
    {
        private long Value;
        private VarInt64(long value)
        {
            Value = value;
        }
        ...
        public async Task DeserializeAsync(SerialStream stream, CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                if ((current & 128) == 0) Value = (long)data;
                position += 7;
                if (position >= sizeof(long) * 8) throw new ProtocolViolationException($"{nameof(VarInt64)} size exceeded!");
            }
        }
        ...
        public static implicit operator long(VarInt64 data)
        {
            return data.Value;
        }
        public static implicit operator VarInt64(long data)
        {
            return new(data);
        }
    }
