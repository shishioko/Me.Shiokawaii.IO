...
{
    public struct VarInt32 : ISerializable, IConvertible
    {
        private int Value;
        private VarInt32(int value)
        {
            Value = value;
        }
        ...
        public async Task DeserializeAsync(SerialStream stream, CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                if ((current & 128) == 0) Value = (int)data;
                position += 7;
                if (position >= sizeof(int) * 8) throw new ProtocolViolationException($"{nameof(VarInt32)} size exceeded!");
            }
        }
        ...
        public static implicit operator int(VarInt32 data)
        {
            return data.Value;
        }
        public static implicit operator VarInt32(int data)
        {
            return new(data);
        }
    }
